...
GitHub Actions Web UI 只能从分支发起 workflow，而本 Release workflow 会有意拒绝分支 ref。如需手动重试发布，请使用 GitHub CLI 从目标 tag 新建运行：
...
`--ref` 与 `release_tag` 必须指向同一个 tag。也可以通过 GitHub API 发起 workflow，并将该 tag 作为请求 ref。
