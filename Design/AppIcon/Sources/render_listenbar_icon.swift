...
private let bodyRect = CGRect(x: 236, y: 432, width: 552, height: 160)
private let dotCenter = CGPoint(x: 334, y: 512)
private let slotRect = CGRect(x: 432, y: 480, width: 250, height: 64)

private func context(size: Int = size) -> CGContext {
...
private func drawSymbol(palette: Palette, in context: CGContext) {
    ...
    context.fillEllipse(in: CGRect(x: dotCenter.x - 34, y: dotCenter.y - 34, width: 68, height: 68))
    fillRoundedRect(slotRect, radius: slotRect.height / 2, color: palette.cutout, in: context)
}
...
private func drawPreview(palette: Palette, output: URL) throws {
    ...
    context.addPath(CGPath(roundedRect: iconRect.insetBy(dx: 6, dy: 6), cornerWidth: 214, cornerHeight: 214, transform: nil))
    context.strokePath()
    context.restoreGState()
...
let iconAssets = try ensureDirectory(root.appendingPathComponent("ListenBar/Resources/AppIcon.icon/Assets").path)
let previews = try ensureDirectory(root.appendingPathComponent("Design/AppIcon/Previews").path)

try drawLayer(palette: defaultPalette, output: iconAssets.appendingPathComponent("listenbar-list-default.png"))
try drawLayer(palette: darkPalette, output: iconAssets.appendingPathComponent("listenbar-list-dark.png"))
try drawLayer(palette: monoPalette, output: iconAssets.appendingPathComponent("listenbar-list-mono.png"))

try drawPreview(palette: defaultPalette, output: previews.appendingPathComponent("listenbar-icon-default.png"))
try drawPreview(palette: darkPalette, output: previews.appendingPathComponent("listenbar-icon-dark.png"))
try drawPreview(palette: monoPalette, output: previews.appendingPathComponent("listenbar-icon-mono.png"))
