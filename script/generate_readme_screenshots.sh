...
DERIVED_DATA_DIR="${TMPDIR:-/tmp}/ListenBarReadmeScreenshotsDerived"
APP_PATH="$DERIVED_DATA_DIR/Build/Products/Debug/ListenBar.app"
BINARY_PATH="$APP_PATH/Contents/MacOS/ListenBar"
RAW_DIR="$ROOT_DIR/Documentation/Screenshots/raw"
OUTPUT_DIR="$ROOT_DIR/Documentation/Screenshots"
MENU_WINDOWS_HELPER="${TMPDIR:-/tmp}/listenbar-readme-menu-windows"
WINDOW_SNAPSHOT="${TMPDIR:-/tmp}/listenbar-readme-window-snapshot"
APP_PID=""
CAPTURE_DIR=""
DISPLAY_ID=""
DISPLAY_LEFT=""
...
if pgrep -x ListenBar >/dev/null 2>&1; then
    fail "Quit all running ListenBar instances before generating screenshots."
fi
...
[[ -x "$BINARY_PATH" ]] || fail "Built ListenBar executable was not found at $BINARY_PATH."
...
display_info="$($MENU_WINDOWS_HELPER display)"
display_fields=("${(@s:,:)display_info}")
[[ ${#display_fields[@]} -eq 7 ]] || fail "Unable to determine the highest-resolution display."
...
wait_for_process() {
    ...
    for attempt in {1..50}; do
        APP_PID="$(pgrep -x ListenBar | head -n 1 || true)"
        [[ -n "$APP_PID" ]] && return 0
        sleep 0.1
    done
    fail "ListenBar did not launch."
}
    ...
    tell application "System Events"
        tell first application process whose unix id is targetPID
            repeat with candidateMenuBar in menu bars
                if exists menu bar item "ListenBar" of candidateMenuBar then
                    set candidateItem to menu bar item "ListenBar" of candidateMenuBar
            ...
            error "ListenBar menu bar item was not found on the selected display."
        end tell
    end tell
end run
APPLESCRIPT
    ...
    fail "ListenBar menu bar item did not appear."
}
            ...
            repeat with candidateMenuBar in menu bars
                if exists menu bar item "ListenBar" of candidateMenuBar then
                    ...
                    if itemWidth > 0 and itemHeight > 0 ¬
                        and centerX ≥ displayLeft and centerX < displayRight ¬
                        and centerY ≥ displayTop and centerY < displayBottom then
                        tell menu item "Terminal" of menu 1 of candidateItem
...
capture_visible_menus() {
    ...
    CAPTURE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/listenbar-readme-window-captures.XXXXXX")"
...
capture_variant() {
    ...
    local actions_path="$RAW_DIR/${name}-actions.png"
    ...
    open_actions_menu
    capture_visible_menus "$actions_path" "$background"
    close_menu
...
expected_files=(
    ...
    "$RAW_DIR/en-light-actions.png"
    "$RAW_DIR/en-dark-main.png"
    "$RAW_DIR/en-dark-actions.png"
    "$RAW_DIR/zh-Hans-light-main.png"
    "$RAW_DIR/zh-Hans-light-actions.png"
    "$RAW_DIR/zh-Hans-dark-main.png"
    "$RAW_DIR/zh-Hans-dark-actions.png"
